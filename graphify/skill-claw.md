...
2. Extract nodes, edges, and hyperedges applying the same rules:
   ...
   - **Controlled edge vocabulary (pick most specific; `related_to` is fallback):** `supports` (A reinforces/evidence B) · `contradicts` (A opposes B) · `depends_on` (A requires B) · `derived_from` (A generated from B) · `part_of` (A component of B) · `preceded_by`/`followed_by` (temporal/order) · `authored_by` (A created by person B) · `tagged_with` (A categorized with B) · `related_to` (fallback only) · `rationale_for` (B explains WHY A) · `references`/`cites` (explicit citation) · `semantically_similar_to` (non-obvious cross-file similarity, INFERRED) · code-legacy: `calls`/`implements` (AST already covers `calls`/`imports`, do NOT duplicate)
